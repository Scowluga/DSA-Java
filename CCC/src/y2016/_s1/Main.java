...
/* Ragaman 15/15
Just practice with Java8 streams
...
public class Main {
    ...
    public static void main(String[] args) {
        try {
            ...
            String[] w1 = reader.readLine().split("");
            String[] w2 = reader.readLine().split("");


            Map<String, Integer> m1 = new HashMap<>();
            Arrays.stream(w1)
                    .collect(Collectors.groupingBy(s -> s))
                    .forEach((k, v) -> m1.put(k, v.size()));

            Map<String, Integer> m2 = new HashMap<>();
            Arrays.stream(w2)
                    .collect(Collectors.groupingBy(s -> s))
                    .forEach((k, v) -> m2.put(k, v.size()));

            int wildcards = m2.getOrDefault("*", 0);
            for (Map.Entry<String, Integer> entry : m1.entrySet()) {
                String s = entry.getKey();
                int i1 = entry.getValue();

                int i2 = m2.getOrDefault(s, 0);

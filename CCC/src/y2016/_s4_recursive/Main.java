...
/* Combining Riceballs 8/15 TLE's last batch
See other file for reasoning
...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int s = 1; s <= N; s++) {
            for (int e = s; e <= N; e++) {
                if (s == e) max = Math.max(max, rb[s]);
                else max = Math.max(max, dp[s][e]);
            }
        }

...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < key.length; i++) {
            shifts[i] = alphabet.indexOf(key[i]);
        }

        String[] word = reader.readLine().replaceAll("[^a-zA-Z]", "").split("");

        for (int i = 0; i < word.length; i++) {
            int index = alphabet.indexOf(word[i]);
            int shift = shifts[i % key.length];

            System.out.print(alphabet.substring(index + shift, index + shift + 1));
